// Shared SELECT projection + joins for post rows, so the chronological listing
// (api/posts) and the personalized ranker (feed/ranking) return identical shapes.

export const POST_BASE_JOINS = `
  FROM posts p
...
/** Column list (without leading SELECT). When authed, adds the viewer's vote + bookmark state. */
export function postColumns(authed: boolean): string {
  return `p.*,
    ...
    COALESCE(cc.cnt, 0) as comment_count${authed ? `,
    uv.direction as user_vote,
    CASE WHEN bm.user_id IS NOT NULL THEN 1 ELSE 0 END as bookmarked` : ""}`
}

/** Viewer-scoped joins. Each `?` binds the viewer's user id (two placeholders when authed). */
export function authJoins(authed: boolean): string {
  return authed
...
export function inflateTags<T extends { tag?: string | null; tags_raw?: string | null }>(post: T) {
