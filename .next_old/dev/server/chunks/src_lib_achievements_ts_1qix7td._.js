module.exports = [
"[project]/src/lib/achievements.ts [instrumentation] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const count = async (db, sql, ...params)=>(await db.get(sql, params))?.c ?? 0;
async function computeAchievementMetrics(db, userId) {
    const user = await db.get("SELECT karma, created_at FROM users WHERE id = ?", [
        userId
    ]);
    const tenureDays = user?.created_at ? Math.max(0, Math.floor((Date.now() - Date.parse(user.created_at)) / 86_400_000)) : 0;
    return {
...
async function checkAndGrantAchievements(db, userId) {
    ...
    for (const def of ACHIEVEMENT_DEFS){
        if (metrics[def.metric] >= def.target) {
            await db.run("INSERT INTO user_achievements (user_id, achievement_id) VALUES (?, ?) ON CONFLICT (user_id, achievement_id) DO NOTHING", [
                userId,
                def.id
            ]);
        }
...
async function getAchievementProgress(db, userId) {
    const metrics = await computeAchievementMetrics(db, userId);
    const defs = new Map(ACHIEVEMENT_DEFS.map((d)=>[
            d.id,
            d
        ]));
    const rows = await db.all("SELECT id, name, description, icon FROM achievements");
...
//# sourceMappingURL=src_lib_achievements_ts_1qix7td._.js.map